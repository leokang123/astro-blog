...
비교 기반 자료구조인 `binary heap`, `red-black tree`, `Fibonacci heap`은 적어도 일부 핵심 연산에 `Ω(lg n)` 장벽이 있다. 하지만 keys가 bounded integers라는 추가 정보를 쓰면 counting sort가 비교 정렬 lower bound를 우회하듯, priority queue/dynamic set operations도 `lg n`보다 빠르게 만들 수 있다. vEB tree는 이 아이디어를 recursive universe decomposition으로 구현한다.
...
SUCCESSOR, PREDECESSOR = O(lg lg u) worst-case
```
...
| `DELETE(x)` | `A[x] = 0` | `O(1)` |
| `MEMBER(x)` | `A[x]` 확인 | `O(1)` |
| `MINIMUM`, `MAXIMUM` | 왼쪽/오른쪽부터 scan | `Θ(u)` |
| `SUCCESSOR`, `PREDECESSOR` | `x` 주변부터 scan | `Θ(u)` |
...
Tree height가 `lg u`이므로 각 operation은 위로 한 번, 아래로 한 번 움직여도 `O(lg u)` worst-case다. 이는 direct bit vector보다 순서 연산에서는 개선이지만, red-black tree의 `O(lg n)`과 비교하면 `n << u`일 때 좋지 않을 수 있다.
...
```text
cluster number = floor(x / sqrt(u))
offset inside cluster = x mod sqrt(u)
```

이 구조에서 `INSERT(x)`는 `A[x] = 1`과 `summary[floor(x/sqrt(u))] = 1`만 하면 되어 `O(1)`이다. 그러나 `MINIMUM`, `MAXIMUM`, `SUCCESSOR`, `PREDECESSOR`, `DELETE`는 여전히 최대 `sqrt(u)` bits짜리 cluster와 summary를 scan해야 하므로 `O(sqrt(u))`이다.
...
이 절의 `proto-vEB` 구조는 이해를 돕기 위한 전 단계다. 여기서는 단순화를 위해 `u = 2^{2^k}`라고 가정한다. 그러면 `u, sqrt(u), u^{1/4}, ...`가 모두 integer power of 2로 깔끔하게 떨어진다. 실제 vEB tree는 20.3에서 이 제한을 완화해 `u=2^k`만 가정한다.
...
| `u >= 4` | `summary` pointer와 `cluster[0..sqrt(u)-1]` pointers를 저장 |
...
|---|---|---|
| binary tree overlay | `u -> u/2` | key range를 반으로 나눔 |
| degree `sqrt(u)` tree | `u -> sqrt(u)`로 볼 준비 | clusters와 summary로 나눔 |
| `proto-vEB` | `u -> sqrt(u)` 재귀 | summary와 each cluster가 같은 ADT를 다시 구현 |
...
Proto-vEB operations는 현재 structure `V`와 key `x`를 받으며, `0 <= x < V.u`를 가정한다. Query operations는 `MEMBER`, `MINIMUM`, `SUCCESSOR`를 중심으로 설명하고, `MAXIMUM`, `PREDECESSOR`는 symmetric하다.
...
```text
u=16: high(6)=1, low(6)=2  -> cluster[1]로 이동
u=4 : high(2)=1, low(2)=0  -> cluster[1]로 이동
u=2 : A[0] = 0             -> 6은 set에 없음
```
...
```text
T(u) = 2T(sqrt(u)) + Θ(lg u)
```

이 recurrence는 `Θ(lg u lg lg u)`가 되어 `MINIMUM`보다도 느리다. proto-vEB가 구조적으로는 좋은 방향이지만 아직 목표를 달성하지 못한다는 사실이 여기서 분명해진다.
...
| `PROTO-vEB-MINIMUM` | 2 | `Θ(lg u)` |
| `PROTO-vEB-SUCCESSOR` | 2 + `MINIMUM` | `Θ(lg u lg lg u)` |
| `PROTO-vEB-INSERT` | 2 | `Θ(lg u)` |
...
```text
upper sqrt: 2^{ceil((lg u)/2)}
lower sqrt: 2^{floor((lg u)/2)}
```

정리본에서는 표기 편의를 위해 각각 `sqrt_up(u)`, `sqrt_down(u)`라고 쓰겠다. 그러면:

```text
u = sqrt_up(u) * sqrt_down(u)
```

`high`, `low`, `index`도 lower square root 기준으로 다시 정의된다.

```text
high(x)  = floor(x / sqrt_down(u))
low(x)   = x mod sqrt_down(u)
index(x, y) = x * sqrt_down(u) + y
```

`summary`는 cluster 번호들을 저장해야 하므로 universe size가 `sqrt_up(u)`이고, 각 cluster는 offset을 저장하므로 universe size가 `sqrt_down(u)`다.
...
```text
{V.min} ∪ 모든 V.cluster[i]에 재귀적으로 저장된 elements
```

단, empty tree에서는 `V.min = V.max = NIL`이고, 원소가 하나뿐이면 `V.min = V.max = 그 원소`이며 clusters에는 아무것도 저장하지 않는다.
...
Odd power of 2일 때 summary는 `sqrt_up(u)` 크기를 갖는다. Recursive procedure들은 모두 최대 하나의 recursive call만 하고, 그 call의 universe size는 많아야 `sqrt_up(u)`다.

```text
T(u) <= T(sqrt_up(u)) + O(1)                         (20.4)
```

`m = lg u`로 두면:

```text
T(2^m) <= T(2^{ceil(m/2)}) + O(1)
```

`ceil(m/2) <= 2m/3` for `m >= 2`이므로:

```text
S(m) <= S(2m/3) + O(1) = O(lg m)
```
...
Recursive call은 최대 하나이고 universe size가 `sqrt_down(u)`로 줄어드므로 recurrence (20.4)에 의해 `O(lg lg u)`다.
...
or
line 15 recursive call does not occur
```
...
| 모든 integer key에 바로 쓸 수 있다 | key가 bounded universe `{0,...,u-1}` 안에 있어야 하며 duplicate도 기본 구조에서는 허용하지 않는다 |
...
1. vEB tree가 comparison-based `Ω(lg n)` 장벽을 우회할 수 있는 전제는 무엇인가?
...
6. Proto-vEB의 `MINIMUM`이 `O(lg lg u)`가 아니라 `Θ(lg u)`인 이유는 무엇인가?
