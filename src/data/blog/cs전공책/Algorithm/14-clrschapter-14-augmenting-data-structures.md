...
```text
x = 26, i = 17, left.size = 12 -> r = 13 -> go right with i = 4
x = 41, i = 4,  left.size = 5  -> r = 6  -> go left with i = 4
x = 30, i = 4,  left.size = 1  -> r = 2  -> go right with i = 2
x = 38, i = 2,  left.size = 1  -> r = 2  -> return 38
```
...
possibly using x.left.f and x.right.f
```
...
`RB-ENUMERATE(x, a, b)`처럼 range 안의 keys를 모두 출력하는 operation은 새 attribute 없이도 `Θ(m + lg n)`에 가능하다. 먼저 lower bound `a` 근처까지 search하고, 이후 successor 방향으로 `b` 이하의 `m`개 keys를 출력하면 된다. 출력 자체가 `m`개이므로 `m` 항은 피할 수 없다.
...
```text
i overlaps i'  <=>  i.low <= i'.high and i'.low <= i.high
```
...
| 경우 | 조건 | 의미 |
| --- | --- | --- |
| overlap | `i.low <= i'.high` and `i'.low <= i.high` | 두 intervals가 겹침 |
...
```text
x.left != T.nil and x.left.max >= i.low
```
...
```text
x = [16,21], no overlap, x.left.max = 23 >= 22 -> go left
x = [8,9],   no overlap, x.left.max = 10 < 22  -> go right
x = [15,23], overlap -> return [15,23]
```

반대로 query `i = [11,14]`는 다음처럼 실패한다.

```text
x = [16,21], no overlap, x.left.max = 23 >= 11 -> go left
x = [8,9],   no overlap, x.left.max = 10 < 11  -> go right
x = [15,23], no overlap, left is T.nil         -> go right
x = T.nil -> return T.nil
```
...
Left로 가는 경우가 더 미묘하다. `x.left.max >= i.low`이면 left subtree 안에 high endpoint가 충분히 큰 interval `i'`가 존재한다. 만약 left subtree에 실제 overlap이 없다면, trichotomy에 의해 이 `i'`는 query `i`의 오른쪽에 있어야 한다.
...
Open intervals를 지원하려면 overlap test의 endpoint inequality를 조정해야 한다. Closed intervals에서는 `i.low <= j.high`와 `j.low <= i.high`를 쓰지만, open intervals에서는 endpoint가 같은 경우 overlap하지 않을 수 있으므로 strict inequality가 필요하다.
...
```text
x.min     = minimum key in subtree rooted at x
x.max     = maximum key in subtree rooted at x
x.min_gap = min(left.min_gap,
                right.min_gap,
                x.key - left.max,
                right.min - x.key)
```
...
```text
x.sum      = subtree endpoint values의 합
x.max_pref = subtree inorder prefix sum의 최대값
x.pom      = max_pref를 달성하는 endpoint
```
...
- `x.max >= i.low`는 left subtree에 overlap이 반드시 있다는 뜻이 아니라, 있을 가능성을 버리면 안 된다는 뜻이다.
