...
order: 8
pubDatetime: 2026-05-14T00:08:00+09:00
modDatetime: 2026-05-14T00:08:00+09:00
...
- Comparison sort는 worst case에서 반드시 $\Omega(n \lg n)$ comparisons가 필요하다.
...
| lower bound | 어떤 알고리즘도 넘을 수 없는 최소 필요 비용 | $\Omega(n \lg n)$ |
...
에 대해 order information을 얻는 유일한 방법이 두 elements 사이의 comparison이다. 즉 $a_i < a_j$, $a_i \le a_j$, $a_i = a_j$, $a_i \ge a_j$, $a_i > a_j$ 같은 test만 사용할 수 있다. 원문은 input elements가 모두 distinct하다고 가정한다. 그러면 equality comparison은 쓸모가 없고, 모든 comparison은 본질적으로
...
Figure 8.1에서 internal node $i:j$는 $a_{i} \le a_{j}$ 비교를 뜻한다. 어떤 input에 대해 sorting algorithm을 실행하면 root에서 시작해 comparison 결과에 따라 leaf까지 내려간다. Leaf에는 예를 들어 $\langle 3,1,2\rangle$처럼 sorted order가 permutation으로 표시된다.
...
$$
\begin{aligned}
Any comparison sort algorithm requires \Omega(n \lg n) \\
\text{comparisons in the worst case.}
\end{aligned}
$$
...
안에 있다고 가정한다. 특히 $k = O(n)$이면 전체 running time이 $\Theta(n)$이 된다.
...
2. 누적합 후 $C[i]$가 $\le i$인 elements 수를 저장한다.
...
$$
\begin{aligned}
\text{initialize C:} \Theta(k) \\
\text{count frequencies:} \Theta(n) \\
\text{prefix sums:} \Theta(k) \\
\text{write output:} \Theta(n)
\end{aligned}
$$
...
Counting sort가 $\Omega(n \lg n)$ lower bound를 깨는 이유는 comparison sort가 아니기 때문이다. Input elements끼리 비교하지 않고, element value를 array index로 사용한다. 이 때문에 key range `0..k`가 너무 크면 memory와 time이 커지고, `k`가 작거나 $O(n)$일 때 빛난다.
...
여기서 stable sort로 counting sort를 쓰는 것이 자연스럽다. 각 digit이 $0..k-1$ 범위라면 한 pass의 비용은 $\Theta(n+k)$다.
...
\begin{aligned}
n개의 d-digit numbers, \\
\text{각 digit이 최대 k개 값 중 하나,} \\
stable sort가 \Theta(n+k)이면 \\
...
- digit 수: $d = \lceil b/r \rceil$
- 각 digit의 가능한 값 수: $2^{r}$
- counting sort 한 pass 비용: $\Theta(n + 2^{r})$
...
| $r \approx \lg n$ | $2^{r} \approx n$이라 counting sort pass가 $\Theta(n)$이고 pass 수가 $b/\lg n$ |

원문은 다음처럼 정리한다.

- $b < \lfloor \lg n \rfloor$이면 $r = b$를 선택해 한 pass로 $\Theta(n)$에 정렬할 수 있다.
- $b \ge \lfloor \lg n \rfloor$이면 $r = \lfloor \lg n \rfloor$이 constant factor 안에서 좋은 선택이며, running time은 $\Theta(bn / \lg n)$이다.

#### Radix sort vs quicksort

겉으로 보면 $b = O(\lg n)$이고 $r \approx \lg n$이면 radix sort는 $\Theta(n)$으로, quicksort의 expected $\Theta(n \lg n)$보다 좋아 보인다. 하지만 실제 선택은 더 복잡하다.
...
Figure 8.4에서 $n = 10$이므로 $B[0]$은 `[0, .1)`, $B[1]$은 `[.1, .2)`, ..., $B[9]$는 `[.9, 1)` 범위를 담당한다. 예를 들어 `.78`은 $\lfloor 10 \cdot .78 \rfloor = 7$이므로 $B[7]$로 들어간다.
...
두 원소 $A[i] \le A[j]$를 생각하자. 그러면

$$
\lfloor n A[i] \rfloor \le \lfloor n A[j] \rfloor
$$
...
$n_{i}$를 bucket $B[i]$에 들어간 원소 수라고 하자. Insertion sort는 bucket 크기에 대해 quadratic time이므로 전체 running time은

$$
T(n) = \Theta(n) + \sum_{i=0}^{n-1} O(n_{i}^{2})
$$

이다. 이제 expectation을 취하면 linearity of expectation에 의해

$$
E[T(n)] = \Theta(n) + \sum_{i=0}^{n-1} O(E[n_{i}^{2}])
$$
...
이 계산은 `indicator random variables`로 보인다. $X_{ij}$를 “$A[j]$가 bucket `i`에 들어가면 1, 아니면 0”인 indicator라고 하면
...
이다. $j \ne k$일 때 두 원소의 bucket 배정은 independent이므로

$$
E[X_{ij} X_{ik}] = E[X_{ij}] E[X_{ik}] = 1/n^{2}
$$
...
\begin{aligned}
E[n_{i}^{2}] \\
= \sum_{j} E[X_{ij}^{2}] + \sum_{j} \sum_{k != j} E[X_{ij} X_{ik}] \\
= n(1/n) + n(n-1)(1/n^{2}) \\
= 1 + (n-1)/n \\
= 2 - 1/n
\end{aligned}
...
이므로 bucket sort의 average-case running time은 $\Theta(n)$이다.
...
- Unit circle 안의 points를 origin으로부터의 distance $d_{i} = \sqrt{x_{i}^{2} + y_{i}^{2}}$ 기준으로 정렬할 때, 점은 면적에 비례해 uniform하다. 반지름 `r` 이하의 면적은 $\pi r^{2}$에 비례하므로 distance 자체가 균등하지 않다. Bucket 경계는 distance가 아니라 area가 균등해지도록 잡아야 한다.
- Continuous probability distribution function $P(x) = \Pr\left\{X \le x\right\}$를 $O(1)$에 계산할 수 있다면, 값 `x`를 $P(x)$로 변환해 `[0,1)`의 uniform variable처럼 다룰 수 있다. 이 idea는 cumulative distribution function, 즉 CDF를 이용한 bucketization이다.
...
| 문제 | 핵심 주제 | 본문과의 연결 |
| --- | --- | --- |
| 8-1 Probabilistic lower bounds on comparison sorting | deterministic/randomized comparison sort의 average lower bound | decision-tree model, $\Omega(n \lg n)$ |
...
| --- | --- | --- | --- |
| Comparison sort lower bound | worst case $\Omega(n \lg n)$ | comparison model | decision tree에 $n!$ reachable leaves 필요 |
| Counting sort | $\Theta(n+k)$ | $A[1..n]$, $B[1..n]$, $C[0..k]$ | key range를 count array index로 사용 |
| Counting sort, $k = O(n)$ | $\Theta(n)$ | integer keys in `0..k` | comparison으로 order를 찾지 않음 |
| Radix sort | $\Theta(d(n+k))$ | `d` digits, digit range $0..k-1$, stable sort 필요 | 낮은 자리부터 digit별 stable sorting |
| Radix sort on $b$-bit keys | $\Theta((b/r)(n + 2^{r}))$ | digit size $r$ bits | pass 수와 counting array 크기의 trade-off |
| Bucket sort average case | $\Theta(n)$ | independent uniform `[0,1)` 또는 $\sum_{i} n_{i}^{2} = O(n)$ | bucket 내부 expected square size가 constant |
| Bucket sort worst case | $\Theta(n^{2})$ | insertion sort 사용 시 | 모든 원소가 한 bucket에 몰릴 수 있음 |
...
- Chapter 6의 heapsort, Chapter 7의 quicksort는 comparison sort 범주에 있으므로 $\Omega(n \lg n)$ lower bound의 적용을 받는다.
...
- $\Omega(n \lg n)$ lower bound는 모든 sorting algorithm에 대한 말이 아니라 `comparison sort`에 대한 말이다.
...
- Radix sort의 $\Theta(n)$ 가능성은 digit 수 `d`와 digit range `k`가 작거나 적절히 제한될 때의 이야기다.
- Bucket sort의 $\Theta(n)$은 average-case 결과다. Worst case는 원소가 한 bucket에 몰리면 $\Theta(n^{2})$이다.
...
2. $n! \le 2^{h}$에서 어떻게 $\Omega(n \lg n)$ lower bound가 나오는가?
...
7. `b`-bit integer를 `r`-bit digit으로 나누면 radix sort의 시간은 왜 $\Theta((b/r)(n + 2^{r}))$인가?
8. Bucket sort의 expected time 분석에서 왜 $E[n_{i}^{2}]$를 계산해야 하는가?
9. Bucket sort의 average-case $\Theta(n)$과 worst-case $\Theta(n^{2})$는 어떤 input distribution 차이에서 발생하는가?
