...
order: 11
pubDatetime: 2026-05-14T00:11:00+09:00
modDatetime: 2026-05-14T00:11:00+09:00
...
Hash table은 worst case에서는 linked list search처럼 $\Theta(n)$까지 나빠질 수 있지만, 적절한 가정 아래 average search time은 $O(1)$이다. 이 장의 핵심은 “key를 array index로 직접 쓰기 어렵거나 낭비가 클 때, hash function으로 key를 table slot에 압축해 대응시킨다”는 것이다.
...
| load factor | table slot 수 대비 저장된 key 수 | $\alpha = n/m$ |
...
Figure 11.1에서는 $U = {0,1,\ldots,9}$이고 실제 key set은 $K = {2,3,5,8}$이다. 따라서 $T[2]$, $T[3]$, $T[5]$, $T[8]$만 element를 가리키고 나머지 slots는 `NIL`이다.
...
\text{U는 매우 큼} \\
\text{K는 작음} \\
\Rightarrow direct-address table은 공간 낭비가 큼
\end{aligned}
$$
...
- Direct-address table에서 maximum을 찾으려면 큰 index부터 scan할 수 있지만 worst case는 $\Theta(m)$이다.
...
여기서 $m$은 hash table $T[0..m-1]$의 slot 수이고, 보통 $m \ll |U|$이다. Key $k$가 slot $h(k)$로 간다고 말하며, $h(k)$를 key $k$의 hash value라고 한다.
...
Figure 11.2에서 $k_{2}$와 $k_{5}$가 같은 slot으로 mapping된다. 이런 상황을 `collision`이라고 한다.
...
Figure 11.3에서는 $h(k_{1}) = h(k_{4})$인 keys가 같은 chain에 있고, $h(k_{5}) = h(k_{7}) = h(k_{2})$인 keys도 같은 chain에 있다.
...
Hash table `T`가 `m` slots를 가지고 `n` elements를 저장한다고 하자. Load factor $\alpha$는 slot 하나당 평균 elements 수다.

$$
\alpha = n / m
$$

Chaining에서는 한 slot에 여러 elements가 들어갈 수 있으므로 $\alpha$는 1보다 클 수도 있다.

각 slot $T[j]$의 chain length를 $n_{j}$라고 하면

$$
\begin{aligned}
n &= n_{0} + n_{1} + \ldots + n_{m-1} \\
E[n_{j}] &= \alpha
\end{aligned}
$$
...
Chaining hash table의 worst case는 모든 keys가 같은 slot으로 hash되는 경우다. 그러면 chain 하나의 길이가 `n`이 되고, search는 $\Theta(n)$이 된다. 이 경우 hash table은 모든 elements를 하나의 linked list에 넣은 것과 다르지 않다.

따라서 ordinary hashing은 worst-case performance를 보고 쓰는 자료구조가 아니다. 평균 성능은 좋지만, worst-case $\Theta(n)$은 남아 있다. Static set에서 worst-case $O(1)$ search를 얻는 방법은 Section 11.5의 `perfect hashing`에서 다룬다.
...
Table에 없는 key `k`를 찾는 unsuccessful search에서는 $T[h(k)]$ chain 끝까지 검사해야 한다. Simple uniform hashing 아래에서 `k`는 어느 slot으로든 동일 확률로 hash되고, 해당 chain의 expected length는 $\alpha$다.

따라서 검사하는 element 수의 expectation은 $\alpha$이고, hash 계산과 slot 접근 $O(1)$까지 포함하면
...
Successful search는 조금 더 미묘하다. 모든 chain이 같은 확률로 검색되는 것이 아니라, 긴 chain일수록 그 안의 element가 많아 검색될 가능성이 더 크다. 그래도 평균 시간은 $\Theta(1+\alpha)$로 나온다.

CLRS의 분석은 insertion order를 사용한다. 새 elements는 chain의 front에 삽입된다. 어떤 element $x_{i}$를 검색할 때 앞에 있는 elements는 $x_{i}$보다 나중에 삽입되었고 같은 slot으로 hash된 elements다.

$x_{i}$의 key를 $k_{i}$라고 하고, indicator random variable을

$$
X_{ij} = I\left\{ h(k_{i}) = h(k_{j}) \right\}
$$
...
$$
\text{successful search} = \Theta(1 + \alpha)
$$
...
- `|U| > nm`이면 어떤 hash function이든 같은 slot으로 몰리는 `n`개 key subset이 존재할 수 있어 worst-case $\Theta(n)$을 피할 수 없다.
...
| 방법 | 성격 | 핵심 |
| --- | --- | --- |
| division method | heuristic | $h(k) = k \bmod m$ |
...
대부분 hash functions는 key universe를 natural numbers $N = {0,1,2,\ldots}$로 본다. Key가 string이면 radix notation으로 자연수처럼 해석한다.
...
예를 들어 대략 $n = 2000$개의 strings를 저장하고 unsuccessful search에서 평균 3개 정도만 보길 원한다면 $\alpha \approx 3$이므로 $m \approx 2000/3$이다. CLRS는 prime $m = 701$을 예로 들고
...
2. Fractional part $kA \bmod 1$만 취하고, 여기에 $m$을 곱한 뒤 floor를 취한다.

$$
h(k) = \lfloor m \cdot (kA \bmod 1) \rfloor
$$

여기서 $kA \bmod 1$은 $kA$의 fractional part, 즉 $kA - \lfloor kA \rfloor$다.
...
꼴로 잡는다. $k$와 $s$를 곱하면 $2w$-bit product가 나오고, 이를 high-order word $r_1$과 low-order word $r_0$로 나눈다.

$$
\text{k * s} = r1 \cdot 2^{w} + r0
$$
...
Fixed hash function은 adversary에게 취약하다. 어떤 hash function `h`가 고정되어 있으면, adversary는 같은 slot으로 hash되는 `n`개 keys를 골라 search time을 $\Theta(n)$으로 만들 수 있다.
...
Hash functions의 finite collection `H`가 있다고 하자. `H`가 universe `U`에서 table slots ${0,1,\ldots,m-1}$로 mapping한다고 할 때, `H`가 universal이라는 뜻은 다음이다.

$$
\begin{aligned}
서로 다른 모든 keys k,l \in U에 대해 \\
Pr_{h randomly chosen from H}[h(k) &= h(l)] \le 1/m
\end{aligned}
$$
...
- Search key `k`가 table에 없으면, `k`가 hash되는 list의 expected length는 at most $\alpha = n/m$이다.
- Search key `k`가 table에 있으면, 그 list의 expected length는 at most $1 + \alpha$이다.

분석은 indicator random variables로 한다. Distinct keys `k`, `l`에 대해

$$
\begin{aligned}
X_{kl} &= I\left\{ h(k) = h(l) \right\} \\
E[X_{kl}] &\le 1/m
\end{aligned}
$$
...
$$
Y_{k} = \sum_{l in T, l != k} X_{kl}
$$

로 두면 linearity of expectation으로 expected collisions가 $\alpha$ 근처로 묶인다. 중요한 점은 이 expectation이 key distribution에 대한 가정이 아니라, random choice of hash function에 대한 expectation이라는 것이다.

#### Corollary 11.4

Universal hashing과 chaining을 쓰고, initially empty table with $m$ slots에서 $O(m)$ insertions만 일어난다면 load factor $\alpha = O(1)$이다. 그러면 어떤 sequence of $n$ `INSERT`, `SEARCH`, `DELETE` operations도 expected $\Theta(n)$ total time에 처리할 수 있다.
...
$$
H_{pm} = \{ h_{ab} : a \in Z_{p}^{*}, b \in Z_{p} \}
$$

이고, 총 $p(p-1)$개의 hash functions가 있다.

Theorem 11.5는 이 $H_{pm}$이 universal임을 보인다. 직관은 다음과 같다.

- 먼저 $(ak + b) \bmod p$ 단계에서는 distinct keys $k \ne l$이 distinct residues $r \ne s$로 간다.
- $p$가 prime이고 $a$와 $k-l$이 nonzero modulo $p$라서 이 단계에서 충돌이 생기지 않는다.
- 마지막 $\bmod m$에서만 collision이 생길 수 있는데, 그 확률이 at most $1/m$으로 bound된다.
...
- $\varepsilon-universal$은 collision probability bound를 $\varepsilon$로 일반화한 개념이다.
...
이다. Chaining에서는 한 slot에 여러 elements가 linked list로 붙을 수 있어 $\alpha > 1$도 가능했지만, open addressing에서는 slot 하나가 element 하나만 담는다.
...
$$
\text{h :} U × {0, 1, \ldots, m-1} \to {0, 1, \ldots, m-1}
$$
...
이다. 모든 slot을 언젠가 검사할 수 있어야 하므로, 각 key에 대해 이 sequence는 ${0,1,\ldots,m-1}$의 permutation이어야 한다.
...
Insertion은 `DELETED` slot을 empty처럼 재사용할 수 있고, search는 `DELETED`를 지나쳐야 한다. 하지만 `DELETED` markers가 쌓이면 search time이 단순히 current load factor $\alpha$만으로 설명되지 않는다. 그래서 deletion이 많은 dictionary에서는 chaining이 더 자주 선택된다.

#### Uniform hashing for open addressing

Open addressing 분석에서는 `uniform hashing`을 가정한다. 이는 각 key의 probe sequence가 ${0,1,\ldots,m-1}$의 `m!` permutations 중 하나로 equally likely하게 선택된다는 이상화된 가정이다.

실제 구현에서 완전한 uniform hashing은 어렵다. CLRS는 다음 세 방법을 소개한다.

| 방법 | probe sequence | clustering |
| --- | --- | --- |
| linear probing | $h(k,i) = (h0(k) + i) \bmod m$ | primary clustering |
| quadratic probing | $h(k,i) = (h0(k) + c1 i + c2 i^{2}) \bmod m$ | secondary clustering |
| double hashing | $h(k,i) = (h1(k) + i h2(k)) \bmod m$ | 가장 random-like |
...
Linear probing보다 primary clustering은 줄지만, 같은 initial probe $h_0(k)$를 가진 keys는 같은 probe sequence를 가진다. 이 현상을 `secondary clustering`이라고 한다. 또한 table 전체를 잘 사용하려면 $c_1$, $c_2$, $m$ 선택에 제약이 있다.
...
Open addressing의 분석은 load factor $\alpha = n/m < 1$에 의존한다. Uniform hashing을 가정하면 unsuccessful search에서 expected probes는

$$
at most 1 / (1 - \alpha)
$$

이다. 직관적으로 첫 probe는 항상 하고, 첫 slot이 occupied일 확률이 대략 $\alpha$, 첫 두 slots가 occupied일 확률이 대략 $\alpha^{2}$이므로
...
이 결과는 insertion에도 바로 적용된다. Insertion은 empty slot을 찾는 unsuccessful search 뒤 그 slot에 key를 쓰는 것이므로, expected probes가 at most $1/(1-\alpha)$이다.
...
| load factor $\alpha$ | unsuccessful search bound | successful search bound |
| --- | --- | --- |
| `0.5` | $1/(1-0.5) = 2$ | `< 1.387` |
| `0.9` | $1/(1-0.9) = 10$ | `< 2.559` |

$\alpha$가 1에 가까워질수록 unsuccessful search와 insertion 비용이 빠르게 커진다. Open addressing은 pointer overhead가 없고 cache locality가 좋을 수 있지만, table이 많이 차면 probe cost가 급격히 나빠진다.
...
2. Second level: slot `j`로 모인 $n_{j}$개 keys를 secondary table $S_{j}$에 다시 hash한다.
3. 각 secondary table에서는 collision이 없도록 hash function $h_{j}$를 고른다.

![Figure 11.6](@/assets/images/cs-algorithm-048-figure-11-6-page-299.png)
*Figure 11.6 · PDF p. 299 · first-level table `T`와 slot별 secondary table $S_{j}$를 사용하는 perfect hashing*

Figure 11.6에서 outer hash function은 key를 primary slot으로 보내고, 각 slot `j`는 자기만의 secondary table $S_{j}$와 secondary hash function $h_{j}$를 가진다. Searching은 두 번의 hash와 두 번의 table access로 끝난다.

$$
\begin{aligned}
j &= h(k) \\
position &= h_{j}(k) \\
\text{check } S_j[\text{position}]
\end{aligned}
$$
...
Slot `j`에 $n_{j}$개 keys가 모였다고 하자. Secondary table size를
...
$$
\begin{aligned}
\text{n keys를 m} &= n^{2} slots에 universal hashing으로 저장하면 \\
...
$$
\begin{aligned}
\text{가능한 key pairs 수: C(n,2)} \\
\text{각 pair collision probability:} &\le 1/m = 1/n^{2} \\
...
Secondary table마다 $m_{j} = n_{j}^{2}$를 쓰면 특정 bucket이 크면 공간이 커질 수 있다. 하지만 first-level hash function을 universal family에서 잘 고르면 expected total secondary size가 linear로 유지된다.

Theorem 11.10은 first-level table size $m = n$일 때

$$
E[\sum_{j=0}^{m-1} n_{j}^{2}] < 2n
$$
...
이다. $\sum \binom{n_{j}}{2}$는 같은 primary slot에 들어간 key pairs, 즉 first-level collisions 수와 같다. Universal hashing에서는 각 pair가 collide할 확률이 at most $1/m = 1/n$이므로 expected collision pairs가 linear로 묶인다.

따라서 Corollary 11.11은

$$
E[\sum m_{j}] = E[\sum n_{j}^{2}] < 2n
$$

이라고 말한다. Primary table 자체, secondary table sizes $m_{j}$, 그리고 hash function parameters $a_{j}$, $b_{j}$까지 포함해도 전체 expected storage는 $O(n)$이다.

Corollary 11.12는 Markov's inequality를 한 번 더 써서

$$
\Pr[\sum m_{j} \ge 4n] < 1/2
$$
...
| Direct-address space | $\Theta(\lvert U \rvert)$ | actual keys가 적으면 낭비 큼 |
...
| Chained hash unsuccessful search | $\Theta(1+\alpha)$ average | simple uniform hashing |
| Chained hash successful search | $\Theta(1+\alpha)$ average | simple uniform hashing |
| Universal hashing with chaining | expected $O(1)$ per operation if $\alpha=O(1)$ | random hash function choice |
| Open addressing unsuccessful search | at most $1/(1-\alpha)$ expected probes | uniform hashing, $\alpha<1$ |
| Open addressing insert | at most $1/(1-\alpha)$ expected probes | unsuccessful search 후 insert |
| Open addressing successful search | at most $(1/\alpha) \ln(1/(1-\alpha))$ expected probes | uniform hashing |
...
- Chapter 31의 modular arithmetic, prime modulus, multiplicative inverse가 universal hash family $H_{pm}$ 증명에 필요하다.
...
- Direct addressing의 $O(1)$은 worst-case지만, 공간이 $\Theta(\lvert U \rvert)$이다.
...
3. Chaining에서 load factor $\alpha = n/m$은 무엇을 의미하는가?
4. Chaining의 unsuccessful search와 successful search가 왜 평균 $\Theta(1+\alpha)$인가?
5. Division method에서 `m`을 power of 2로 잡는 것이 왜 위험할 수 있는가?
6. Multiplication method에서 $h(k) = \operatorname{floor}(m(kA \bmod 1))$의 의미를 설명하라.
...
10. Perfect hashing에서 secondary table size를 $m_{j} = n_{j}^{2}$로 잡는 이유와 전체 공간이 expected $O(n)$인 이유를 설명하라.
