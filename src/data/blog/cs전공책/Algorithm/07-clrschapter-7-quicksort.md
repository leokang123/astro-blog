...
Chapter 7은 `quicksort`를 다룬다. Quicksort는 worst-case running time이 `Θ(n^2)`이지만, average/expected behavior가 매우 좋고 hidden constant가 작아 practical sorting에서 강력한 선택지가 된다. 또한 `in place`로 동작하고 virtual-memory environments에서도 잘 작동한다고 원문은 설명한다.
...
| worst-case partitioning | partition이 `0`과 `n-1`로 갈라지는 경우 | Θ(n^2) |
| balanced partitioning | subproblem sizes가 일정 비율로 나뉘는 경우 | Θ(n lg n) |
...
- `A[j] <= x`: `i`를 증가시키고 `A[i]`와 `A[j]`를 swap한다. 그러면 새 `A[i]`는 `<= x` 영역에 들어가고, swap으로 밀려난 값은 기존 invariant에 의해 `> x` 영역에 남는다. 이후 `j`가 증가한다.

Termination:

Loop가 끝나면 `j = r`이다. 따라서 `A[p..i] <= x`, `A[i+1..r-1] > x`, `A[r] = x`가 성립한다. 마지막으로 pivot `A[r]`를 `A[i+1]`과 swap하면 pivot은 왼쪽의 모든 값보다 크거나 같고 오른쪽의 모든 값보다 작거나 같은 위치에 놓인다. 반환값은
...
원문은 이보다 조금 더 강하게, 오른쪽 영역의 모든 값이 pivot보다 strictly greater라고 설명한다. 기본 `PARTITION`이 `<= x`를 왼쪽으로 보내기 때문이다.
...
Quicksort의 running time은 `PARTITION`이 얼마나 balanced한 subproblems를 만드는지에 달려 있다. Balanced partition이면 merge sort처럼 빠르고, maximally unbalanced partition이면 insertion sort처럼 `Θ(n^2)`까지 느려질 수 있다.

핵심 recurrence는 항상 다음 형태다.

```text
T(n) = T(left size) + T(right size) + Θ(n)
```

여기서 `Θ(n)`은 `PARTITION`이 subarray를 한 번 scan하는 비용이다. 따라서 partition sizes가 recursion tree의 height와 level cost를 결정한다.
...
중요한 점은 이 worst case가 흔한 입력에서도 발생할 수 있다는 것이다. 기본 `PARTITION`이 항상 마지막 원소 `A[r]`를 pivot으로 쓰기 때문에, array가 이미 increasing order로 sorted되어 있으면 pivot은 항상 maximum이고 split은 `n-1`과 `0`이 된다. 이 경우 insertion sort는 `O(n)`이지만 quicksort는 `Θ(n^2)`가 된다.
...
이다. Perfect balance가 반복되면 recursion tree height는 `Θ(lg n)`이고, 각 level의 total partition cost는 `Θ(n)`이기 때문이다.
...
- 가장 작은 쪽은 깊이 `log_10 n` 정도에서 boundary condition에 도달한다.
...
```text
Pr{chosen pivot is any fixed element in A[p..r]} = 1 / (r-p+1)
```
...
1. `QUICKSORT`와 `RANDOMIZED-QUICKSORT` 모두 worst-case running time은 `Θ(n^2)`이다.
2. `RANDOMIZED-QUICKSORT`의 expected running time은 distinct elements 가정 아래 `O(n lg n)`이고, best-case lower bound와 합치면 `Θ(n lg n)`이다.
...
이다. `T(n) <= c n^2`라고 guess하고 substitution method를 적용한다.

```text
T(n)
<= max_{0 <= q <= n-1} (c q^2 + c(n-q-1)^2) + Θ(n)
= c max_{0 <= q <= n-1} (q^2 + (n-q-1)^2) + Θ(n)
```
...
상수 `c`를 충분히 크게 잡으면 `c(2n-1)`이 `Θ(n)` 항을 지배한다. 따라서

```text
T(n) = O(n^2)
```

이고, Section 7.2에서 이미 unbalanced partition이 계속되는 구체적 경우가 `Ω(n^2)`임을 보았으므로 worst-case running time은
...
여기서 `z_i`는 `i`번째로 작은 element다. 또한

```text
Z_ij = {z_i, z_{i+1}, ..., z_j}
```

를 `z_i`와 `z_j` 사이의 elements 집합으로 둔다.
...
```text
X = Σ_{i=1}^{n-1} Σ_{j=i+1}^{n} X_ij
```

linearity of expectation과 `E[I{A}] = Pr{A}`를 쓰면

```text
E[X]
= Σ_{i=1}^{n-1} Σ_{j=i+1}^{n} E[X_ij]
= Σ_{i=1}^{n-1} Σ_{j=i+1}^{n} Pr{z_i is compared to z_j}
```

#### 두 element는 언제 비교되는가

`z_i`와 `z_j`가 비교되는 조건은 `Z_ij` 안에서 가장 먼저 pivot으로 선택되는 element가 `z_i` 또는 `z_j`인 경우다.

왜 그런가?

- 만약 `Z_ij` 내부의 어떤 중간 element `z_k` (`i < k < j`)가 먼저 pivot으로 선택되면, `z_i`와 `z_j`는 서로 다른 partitions로 갈라진다. 이후에는 절대 비교되지 않는다.
- 반대로 `z_i`가 `Z_ij`에서 가장 먼저 pivot이 되면, 그 partition call에서 `z_i`는 `Z_ij`의 다른 elements와 비교되므로 `z_j`와도 비교된다.
- `z_j`가 먼저 pivot이 되는 경우도 마찬가지다.

`Z_ij`에는 `j-i+1`개 elements가 있고, randomized pivot selection 때문에 그중 어느 element가 first pivot이 될 확률은 모두 같다. 따라서

```text
Pr{z_i is compared to z_j}
= Pr{z_i or z_j is first pivot chosen from Z_ij}
= 1/(j-i+1) + 1/(j-i+1)
= 2/(j-i+1)
```
...
이다. Section 7.2의 best-case lower bound `Ω(n lg n)`와 Exercise 7.4-4의 expected lower bound를 함께 보면 randomized quicksort의 expected running time은
...
- `7-1 Hoare partition correctness`: CLRS Section 7.1의 `PARTITION`은 Lomuto partition이고, 원래 Hoare가 제시한 `HOARE-PARTITION`은 다른 방식이다. Hoare partition은 pivot을 두 partitions 중 하나에 남기며, 반환 index `j`가 `p <= j < r`인 nontrivial split을 보장한다.
...
- `7-3 Alternative quicksort analysis`: comparison count 대신 recursive call의 expected running time recurrence로 `E[T(n)] = Θ(n lg n)`을 보이는 접근이다.
- `7-4 Stack depth for quicksort`: tail recursion을 이용하면 두 번째 recursive call을 loop로 바꿀 수 있다. 항상 작은 subarray를 먼저 recursive하게 처리하도록 바꾸면 worst-case stack depth를 `Θ(lg n)`으로 줄일 수 있다.
- `7-5 Median-of-3 partition`: 세 random elements의 median을 pivot으로 쓰면 median 근처 pivot을 선택할 확률이 증가해 constant factor가 개선된다. 하지만 comparison sort lower bound 관점에서 asymptotic `Θ(n lg n)`은 그대로다.
...
Chapter notes는 quicksort가 Hoare가 발명한 알고리즘이며, Section 7.1의 `PARTITION`은 Lomuto partition임을 언급한다. 또한 quicksort 구현 세부는 실제 성능에 큰 영향을 미치며, adversary가 random choices를 본 뒤 input을 구성할 수 있다면 randomized implementation도 `Θ(n^2)` 실행을 유도당할 수 있음을 지적한다.

## 복잡도

| 대상 | 시간 / 기대값 | 핵심 이유 |
| --- | --- | --- |
| `PARTITION(A,p,r)` | `Θ(r-p+1)` | subarray를 한 번 scan |
| `QUICKSORT` worst case | `Θ(n^2)` | 매번 `(0, n-1)` split |
| `QUICKSORT` best case | `Θ(n lg n)` | 매번 balanced split |
| constant proportional split | `O(n lg n)` | recursion depth가 `Θ(lg n)`, level cost `O(n)` |
| `RANDOMIZED-PARTITION` | `Θ(n)` | random pivot swap + normal partition |
| `RANDOMIZED-QUICKSORT` expected | `O(n lg n)` | expected comparisons `O(n lg n)` |
| `RANDOMIZED-QUICKSORT` worst case | `Θ(n^2)` | pivot choices가 계속 unlucky할 수 있음 |
...
- Chapter 6의 heapsort와 비교된다. Heapsort는 worst-case `O(n lg n)`과 in-place를 보장하고, quicksort는 worst-case `Θ(n^2)`이지만 평균적으로 매우 빠르고 practical constant가 작다.
...
- Quicksort가 항상 `O(n lg n)`인 것은 아니다. 기본 worst case는 `Θ(n^2)`이다.
- Randomized quicksort도 worst-case running time 자체는 `Θ(n^2)`다. 다만 expected running time이 좋다.
...
3. 기본 quicksort가 sorted input에서 `Θ(n^2)`가 되는 이유를 설명하라.
...
7. `z_i`와 `z_j`가 비교되는 정확한 조건은 무엇인가?
8. 왜 `Pr{z_i is compared to z_j} = 2/(j-i+1)`인가?
