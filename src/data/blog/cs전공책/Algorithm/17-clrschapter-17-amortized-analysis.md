...
| `potential method` | credit을 개별 object가 아니라 data structure 전체의 potential energy `Φ`로 저장한다. |
...
```text
total cost of n stack operations = O(n)
...
`c_i`를 `i`번째 operation의 actual cost, `ĉ_i`를 amortized cost라고 하자. 모든 operation sequence와 모든 prefix length `n`에 대해 다음이 성립해야 한다.

```text
Σ_{i=1}^{n} ĉ_i >= Σ_{i=1}^{n} c_i                         (17.1)
```

두 합의 차이가 data structure에 저장된 total credit이다.

```text
total credit = Σ amortized costs - Σ actual costs
```
...
```text
credit on each object currently in stack = 1
total credit = current stack size >= 0
```
...
Binary counter에서는 bit flip 하나를 cost 1로 본다. Accounting method는 bit를 `0 -> 1`로 set할 때 2 dollars를 charge한다.

| Bit transition | Charge | 실제 지불 | 저장/사용 |
|---|---:|---:|---|
| `0 -> 1` | 2 | 1 | 남은 1 dollar를 그 bit 위에 credit으로 저장 |
| `1 -> 0` | 0 | 1 | bit 위의 1 dollar credit으로 reset 비용 지불 |

이때 invariant는 다음과 같다.

```text
every bit whose value is 1 has 1 dollar of credit
total credit = number of 1s in the counter >= 0
```
...
total actual cost of n INCREMENT operations <= total amortized cost = O(n)
```
...
| binary counter | bit `0 -> 1` set | bit value가 1인 position | future `1 -> 0` reset 비용 |
...
초기 data structure를 `D_0`라고 하자. `i`번째 operation 후의 data structure를 `D_i`, actual cost를 `c_i`라고 한다. Potential function `Φ`는 각 data structure state `D_i`를 real number `Φ(D_i)`로 보낸다.

Potential method에서 `i`번째 operation의 amortized cost `ĉ_i`는 다음과 같다.

```text
ĉ_i = c_i + Φ(D_i) - Φ(D_{i-1})                         (17.2)
```
...
```text
Σ_{i=1}^{n} ĉ_i
= Σ_{i=1}^{n} (c_i + Φ(D_i) - Φ(D_{i-1}))
= Σ_{i=1}^{n} c_i + Φ(D_n) - Φ(D_0)                     (17.3)
```

따라서 `Φ(D_n) >= Φ(D_0)`이면:

```text
Σ amortized costs >= Σ actual costs
```

실제로는 operations 수를 미리 모르는 경우가 많으므로, 보통 `Φ(D_0) = 0`으로 정의하고 모든 `i`에 대해 `Φ(D_i) >= 0`임을 보인다. 그러면 모든 prefix sequence에 대해 amortized total이 actual total의 upper bound가 된다.
...
Potential function은 하나만 정답인 것이 아니다. 어떤 `Φ`를 잡느냐에 따라 amortized bounds가 달라질 수 있고, 좋은 potential function은 자료구조가 “비싼 operation 직전까지 얼마나 에너지를 모았는가”를 잘 표현한다.
...
```text
Φ(D_i) = number of objects in stack after i-th operation
```

Initially empty stack이므로 `Φ(D_0) = 0`이고, stack size는 음수가 될 수 없으므로 모든 `i`에 대해 `Φ(D_i) >= 0`이다.
...
```text
Φ(D_i) = b_i = number of 1s after i-th INCREMENT
```

`i`번째 `INCREMENT`가 `t_i`개의 1 bits를 0으로 reset한다고 하자. 그 뒤 최대 하나의 bit를 1로 set하므로 actual cost는:

```text
c_i <= t_i + 1
```

Reset된 `t_i`개의 1 bits 때문에 potential은 줄고, 새로 set되는 bit 때문에 많아야 1 증가한다.

```text
b_i <= b_{i-1} - t_i + 1
Φ(D_i) - Φ(D_{i-1}) <= 1 - t_i
```
...
Counter가 0에서 시작하면 `Φ(D_0) = 0`, 그리고 항상 `Φ(D_i) >= 0`이므로 `n`번 `INCREMENT`의 total actual cost는 `O(n)`이다.

#### Counter가 0에서 시작하지 않는 경우

Potential method는 초기 state가 0이 아닐 때도 깔끔하다. Counter가 처음에 `b_0`개의 1 bits를 가지고, `n` operations 후 `b_n`개의 1 bits를 가진다고 하자. Equation (17.3)을 actual cost 기준으로 재배열하면:

```text
Σ_{i=1}^{n} c_i = Σ_{i=1}^{n} ĉ_i - Φ(D_n) + Φ(D_0)             (17.4)
```

각 `ĉ_i <= 2`, `Φ(D_0)=b_0`, `Φ(D_n)=b_n`이므로:

```text
Σ c_i <= 2n - b_n + b_0
```

특히 `b_0 <= k`이고 `k = O(n)`이면 total actual cost는 여전히 `O(n)`이다. 더 일반적으로 충분히 많은 increments를 수행해 초기 1 bits가 만들어 둔 potential 효과가 희석되면 amortized bound가 유지된다.
...
| 핵심 조건 | total credit never negative | `Φ(D_i) >= Φ(D_0)` 또는 `Φ(D_i) >= 0` |
| 계산 방식 | operation별 charge와 credit 사용 설명 | `c_i + ΔΦ`로 수식 계산 |
...
| `α(T) = T.num / T.size` | `load factor` |

비어 있지 않은 table에서는 `α(T)=T.num/T.size`이고, empty table에서는 편의상 `T.size=0`, `α(T)=1`로 둔다. `load factor`가 어떤 positive constant 아래로 떨어지지 않도록 보장하면 unused space는 전체 table size의 constant fraction 안에 묶인다.

#### 17.4.1 Table expansion

먼저 deletion이 없고 insertion만 있다고 하자. Table이 가득 찬 상태(`T.num == T.size`, 즉 `α(T)=1`)에서 새 object를 넣으려면 더 큰 table을 새로 allocate하고 기존 object를 모두 copy한 뒤 old table을 free해야 한다. 가장 흔한 전략은 table size를 두 배로 늘리는 `doubling`이다.
...
`expansion`은 line 5-9의 재할당과 copy 과정이다. Cost model은 한 object 삽입 비용을 1로 보고, allocation/free overhead는 copied objects 수에 의해 dominate된다고 본다. 따라서 `i`번째 insertion이 expansion을 일으키지 않으면 `c_i=1`이고, expansion을 일으키면 기존 `i-1`개를 copy하고 새 item을 insert하므로 `c_i=i`가 된다.
...
따라서 `TABLE-INSERT`의 amortized cost는 at most 3, 즉 `O(1)`이다. Worst-case single insertion은 expansion 때문에 `Θ(n)`일 수 있지만, 긴 sequence 전체에서는 expensive operation이 충분히 sparse하게 나타난다.
...
| 경우 | Actual cost `c_i` | Potential change | Amortized cost |
|---|---:|---:|---:|
| expansion 없음 | `1` | `+2` | `3` |
...
Deletion까지 지원하려면 table이 지나치게 비었을 때 `contraction`도 해야 한다. 하지만 단순히 “full이면 double, delete 후 half보다 작아지면 halve”로 하면 좋지 않다. Table size가 `n/2`이고 full인 상태에서 insert 한 번으로 size `n`으로 expand된 뒤, delete 몇 번으로 곧바로 size `n/2`로 contract될 수 있다. 그 다음 insert들이 다시 expansion을 일으키면 expansion/contraction이 반복되어 `Θ(n)` cost operation이 연속적으로 발생하고, `n` operations가 `Θ(n^2)` total cost까지 커질 수 있다.

해결책은 expansion과 contraction 사이에 간격을 두는 `hysteresis`다.

| 상황 | 동작 |
|---|---|
| `α(T)=1`에서 insert | table size를 double |
| delete 후 `α(T) < 1/4` | table size를 halve |
| 그 외 | size 유지 |
...
Insertion-only potential `2*T.num - T.size`는 `α < 1/2`에서 negative가 될 수 있으므로 deletion을 함께 분석하려면 potential을 바꿔야 한다. 책은 다음 piecewise potential을 사용한다.

```text
Φ(T) =
    2 * T.num - T.size      if α(T) >= 1/2
    T.size / 2 - T.num      if α(T) < 1/2                    (17.6)
```
...
| Operation | 조건 | 핵심 결론 |
|---|---|---|
| `TABLE-INSERT` | `α_{i-1} >= 1/2` | 17.4.1과 같은 방식으로 amortized cost `<= 3` |
| `TABLE-INSERT` | `α_{i-1} < 1/2`, 여전히 `α_i < 1/2` | actual cost 1이지만 potential이 1 감소해 amortized cost 0 |
| `TABLE-INSERT` | `α_{i-1} < 1/2`, insertion 후 `α_i >= 1/2` | 경계 `1/2`을 넘는 순간에도 amortized cost는 constant, 실제 계산상 `< 3` |
| `TABLE-DELETE` | `α_{i-1} < 1/2`, contraction 없음 | actual cost 1과 potential 증가를 합쳐 amortized cost 2 |
...
```text
T.size_i / 2 = T.size_{i-1} / 4 = T.num_i + 1
```

즉 contraction 직전에는 table이 `1/4` threshold를 막 지나고 있었고, contraction 후에는 load factor가 다시 약 `1/2`가 된다. 이 순간 potential이 크게 줄어 expensive copying을 지불한다. `α_{i-1} >= 1/2`인 delete의 상세 계산은 연습문제로 남겨져 있지만, 같은 potential로 constant amortized bound가 나온다.
...
| `17-3 Amortized weight-balanced trees` | subtree가 `α-balanced` 조건을 깨면 rebuild하고, imbalance가 쌓이는 정도를 potential로 해석 |
...
| 어떤 operation도 실제로 싸다는 뜻이다 | 아니다. 개별 operation은 `Θ(n)`일 수 있지만, 그런 operation이 자주 반복될 수 없음을 보인다 |
| Accounting method의 credit은 실제 메모리나 필드다 | 분석을 위한 가상 장부일 뿐이며 implementation에 저장할 필요가 없다 |
| Potential function은 아무 nonnegative 함수면 된다 | `c_i + ΔΦ`가 원하는 bound를 주도록 state 변화와 잘 맞아야 한다 |
...
5. Potential method의 식 `ĉ_i = c_i + Φ(D_i) - Φ(D_{i-1})`가 왜 telescoping되는가?
