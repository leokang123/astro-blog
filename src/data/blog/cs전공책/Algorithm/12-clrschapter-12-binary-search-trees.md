...
order: 12
pubDatetime: 2026-05-14T00:12:00+09:00
modDatetime: 2026-05-14T00:12:00+09:00
...
이 장의 핵심 경고는 간단하다. Binary search tree의 기본 연산 시간은 tree의 node 수가 아니라 `height h`에 비례한다. 같은 key 집합이라도 tree가 균형 있게 생기면 높이가 $\Theta(\lg n)$이고, 한쪽으로 길게 늘어진 linear chain이 되면 높이가 $\Theta(n)$이다. 따라서 plain BST는 자동으로 빠른 구조가 아니라, 입력 순서와 tree shape에 성능이 크게 의존하는 구조다.
...
즉 node $x$를 기준으로 왼쪽 subtree에는 $x.key$ 이하의 key만, 오른쪽 subtree에는 $x.key$ 이상의 key만 온다. 이 조건은 root에서만 성립하면 되는 것이 아니라 모든 node에 대해 재귀적으로 성립해야 한다. CLRS의 부등호가 $\le$, $\ge$인 점도 중요하다. Plain BST 정의 자체는 duplicate keys를 배제하지 않으며, 같은 key를 어느 방향에 둘지는 구현 정책과 이후 문제에서 다시 다룬다.
...
`Theorem 12.1`은 node `x`가 `n`-node subtree의 root일 때 `INORDER-TREE-WALK(x)`가 $\Theta(n)$ time을 쓴다고 말한다.

핵심 이유는 두 가지다.

- Lower bound: 모든 node의 key를 실제로 출력해야 하므로 적어도 $\Omega(n)$이다.
...
이고 substitution method로 $T(n) \le (c+d)n + c$를 보여 $O(n)$을 얻는다. 따라서 전체적으로 $\Theta(n)$이다. 중요한 결론은 inorder walk의 시간은 tree height에 의존하는 query/update operation과 달리, 모든 node를 방문해야 하므로 tree shape와 무관하게 linear time이라는 점이다.
...
또한 arbitrary list에서 comparison-based 방식으로 BST를 만들어 그 inorder traversal이 sorted list가 되게 하려면, 결과적으로 comparison sorting을 수행하는 것과 같으므로 worst case $\Omega(n \lg n)$ lower bound를 피할 수 없다.
...
공통 이유는 모든 절차가 downward path, upward path, 또는 둘의 짧은 조합 하나만 따라가기 때문이다. 이 절의 $O(h)$는 balance 보장이 아니라 path length에 대한 bound다. 따라서 tree가 linear chain이면 $h = \Theta(n)$이 되어 query도 worst case $\Theta(n)$까지 나빠진다.
...
`TREE-SUCCESSOR`를 반복 호출해 inorder order를 만들면 각 호출이 개별적으로는 $O(h)$이지만 전체 traversal은 $\Theta(n)$이 될 수 있다. 각 edge가 아래로 내려가거나 위로 올라가는 방향으로 전체 과정에서 제한된 횟수만 지나가기 때문이다. 이 관찰은 amortized reasoning의 작은 예고편이다.
...
BST sort는 `TREE-INSERT`를 반복해 tree를 만들고 `INORDER-TREE-WALK`로 출력하는 sorting method다. Best case에는 삽입 순서가 tree를 균형 있게 만들어 build가 $\Theta(n \lg n)$이고 inorder walk가 $\Theta(n)$이라 전체 $\Theta(n \lg n)$이다. Worst case에는 삽입 순서가 chain을 만들어 build가 $\Theta(n^{2})$이고, inorder walk까지 합쳐도 $\Theta(n^{2})$이다.
...
지금까지 BST 기본 연산은 모두 $O(h)$ time이었다. 따라서 남은 핵심 질문은 height $h$가 얼마나 커지는가다. $n$개 key를 strictly increasing order로 삽입하면 tree는 height $n-1$인 chain이 된다. 반대로 어떤 binary tree든 height는 적어도 $\lfloor \lg n \rfloor$ 이상이다. 평균적 동작은 quicksort처럼 worst case보다 best case에 훨씬 가깝다.
...
| 기호 | 의미 |
| --- | --- |
| $X_{n}$ | $n$개 key로 만든 randomly built BST의 height |
| $Y_{n} = 2^{X_{n}}$ | exponential height |
| $R_{n}$ | root key의 rank, 즉 sorted order에서 root가 몇 번째 key인지 |
| $Z_{n,i} = I\left\{R_{n} = i\right\}$ | root rank가 $i$인지 나타내는 indicator random variable |

Root는 random insertion order의 첫 key이므로 rank $R_{n}$은 $\{1, 2, \ldots, n\}$에서 균등하다. 만약 $R_{n} = i$이면 left subtree는 rank가 작은 $i-1$개 key로, right subtree는 rank가 큰 $n-i$개 key로 randomly built BST가 된다. Height는 두 subtree height 중 큰 값에 1을 더한 것이므로 exponential height는 다음처럼 쓸 수 있다.

$$
\begin{aligned}
Y_{n} &= 2 \cdot \max(Y_{i-1}, Y_{n-i}) when R_{n} = i \\
Y_{1} &= 1, Y_{0} = 0
\end{aligned}
$$
...
$$
\begin{aligned}
Y_{n} &= \sum_{i=1}^{n} Z_{n,i} \cdot (2 \cdot \max(Y_{i-1}, Y_{n-i})) \\
E[Z_{n,i}] &= 1/n (12.1)
\end{aligned}
$$
...
\begin{aligned}
E[Y_n] \\
  &\le (2/n) \sum_{i=1}^{n} (E[Y_{i-1}] + E[Y_{n-i}]) \\
  &= (4/n) \sum_{i=0}^{n-1} E[Y_{i}] (12.2)
\end{aligned}
$$
...
$$
\sum_{i=0}^{n-1} \binom{i+3}{3} = \binom{n+3}{4} (12.3)
$$

을 이용해

$$
E[Y_{n}] \le (1/4) \cdot \binom{n+3}{3}
$$

임을 보인다. 오른쪽은 $\Theta(n^{3})$인 polynomial bound다.

마지막 단계에서 Jensen's inequality를 쓴다. $f(x) = 2^{x}$는 convex이므로,

$$
2^{E[X_{n}]} \le E[2^{X_{n}}] = E[Y_{n}] \le (1/4) \cdot \binom{n+3}{3} = O(n^{3})
$$
...
이 된다. 이 증명의 미묘한 점은 $E[X_{n}]$를 직접 recurrence로 잡지 않고 $E[2^{X_{n}}]$를 polynomial로 억제한 뒤 convexity로 height expectation을 끌어낸다는 것이다.

#### 12.4 Exercises에서 남길 포인트

Average depth가 $\Theta(\lg n)$이어도 height가 반드시 $\Theta(\lg n)$인 것은 아니다. 대부분의 node가 얕은 곳에 있고, 소수 node만 긴 tail을 이루면 average depth는 작게 유지되면서 height는 더 커질 수 있다. 따라서 average depth bound와 height bound는 같은 말이 아니다.
...
Equal keys는 plain `TREE-INSERT`에서 나쁜 shape를 만들기 쉽다. CLRS pseudocode는 `z.key < x.key`가 아니면 right로 가므로, identical keys를 계속 삽입하면 모두 한쪽으로 이어진 chain이 되어 총 insertion time이 $\Theta(n^{2})$가 된다.
...
Figure 12.5는 bit strings `1011`, `10`, `011`, `100`, `0`을 저장한다. Heavily shaded nodes는 실제 key가 아니라 다른 key로 가는 path를 만들기 위해 필요한 내부 node다. Lexicographic sort는 radix tree를 왼쪽부터 traversal하면서 실제 key가 있는 node를 출력하면 된다. 모든 string 길이의 합이 `n`이면 tree를 만들고 traversal하는 총 작업도 $\Theta(n)$ time에 가능하다.
...
Left/right subtrees를 $T_{L}$, $T_{R}$라 할 때, root를 제외한 모든 node는 subtree 안에서보다 depth가 1 증가하므로
...
$$
\begin{aligned}
P(n) &= (1/n) \sum_{i=0}^{n-1} (P(i) + P(n-i-1) + n - 1) \\
     &= (2/n) \sum_{k=1}^{n-1} P(k) + \Theta(n)
\end{aligned}
$$
...
$b_{n}$을 `n`개 node를 가진 서로 다른 binary tree 수라고 하자. Root의 left subtree 크기가 `k`이면 right subtree 크기는 $n-1-k$이므로,
...
를 얻는다. Generating function $B(x) = \sum_{n=0}^{\infty} b_{n} x^{n}$를 쓰면
...
$$
\begin{aligned}
b_{n} &= (1/(n+1)) \cdot \binom{2n}{n} \\
b_{n} &= 4^{n} / \sqrt{\pi n^{3}} \cdot (1 + O(1/n))
\end{aligned}
$$
...
| --- | --- | --- |
| `INORDER-TREE-WALK` | $\Theta(n)$ | 모든 node를 한 번씩 방문하고 출력 |
...
| worst-case BST height | $\Theta(n)$ | increasing order insertion 등으로 chain 가능 |
...
9. Randomly built BST의 expected height가 $O(\lg n)$이라는 증명의 핵심 random variables $X_{n}$, $Y_{n}$, $R_{n}$은 무엇인가?
