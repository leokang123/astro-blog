...
order: 4
pubDatetime: 2026-05-14T00:04:00+09:00
modDatetime: 2026-05-14T00:04:00+09:00
...
Recurrence는 divide-and-conquer와 자연스럽게 따라다닌다. 어떤 algorithm이 size $n$ 문제를 더 작은 input에 대한 recursive calls로 풀면, running time $T(n)$도 더 작은 크기의 $T(\ldots)$를 포함하는 식으로 표현된다. 예를 들어 merge sort의 worst-case running time은 다음 recurrence로 표현된다.
...
Recurrence는 다양한 형태를 가질 수 있다. Unequal split이면 $T(n) = T(2n/3) + T(n/3) + \Theta(n)$처럼 되고, recursive linear search처럼 input을 하나만 줄이면 $T(n) = T(n - 1) + \Theta(1)$이 된다.
...
Recurrence를 쓸 때는 floors, ceilings, boundary conditions를 종종 생략한다. 예를 들어 실제 merge sort는 odd $n$에서 $T(\lceil n/2 \rceil) + T(\lfloor n/2 \rfloor) + \Theta(n)$이지만, 보통 $2T(n/2) + \Theta(n)$으로 쓴다. $T(1)$ 같은 작은 input의 정확한 값도 생략하는데, 이는 보통 exact solution만 constant factor만큼 바꾸고 asymptotic order는 바꾸지 않기 때문이다. 다만 이런 technicalities가 항상 무해한 것은 아니므로, 어떤 경우에 영향이 없는지 판단하는 감각이 필요하다.
...
Brute-force solution은 모든 buy/sell date pair, 또는 모든 contiguous subarray를 검사한다. $n$일에 대해 가능한 pair는 $\Theta(n^{2})$개이고, subarray sums를 이전 계산값을 이용해 $O(1)$에 갱신하더라도 전체는 $\Theta(n^{2})$ time이다. 목표는 이를 $o(n^{2})$로 개선하는 것이다.
...
| 경우 | 조건 | 해결 방식 |
| --- | --- | --- |
| entirely in left | $low \le i \le j \le mid$ | $A[low..mid]$에서 recursive solve |
| entirely in right | $mid < i \le j \le high$ | $A[mid+1..high]$에서 recursive solve |
| crossing midpoint | $low \le i \le mid < j \le high$ | 왼쪽 suffix와 오른쪽 prefix를 linear scan으로 결합 |
...
두 loops의 iteration 수는 $(mid - low + 1) + (high - mid) = high - low + 1 = n$이므로 running time은 $\Theta(n)$이다.
...
Recursive case에서는 left/right 두 subproblems가 각각 size $n/2$이고, crossing subarray 계산이 $\Theta(n)$이다. 나머지 comparisons와 tuple returns는 $\Theta(1)$이다.

$$
T(n) = 2T(n/2) + \Theta(n)
$$

이 recurrence는 merge sort와 동일하며, master method 또는 Figure 2.5식 recursion tree로 $T(n) = \Theta(n \lg n)$을 얻는다. 따라서 divide-and-conquer maximum-subarray algorithm은 brute-force $\Theta(n^{2})$보다 asymptotically faster하다.

단, 이것이 이 문제의 최선은 아니다. Exercise 4.1-5는 왼쪽에서 오른쪽으로 훑으며 “지금까지 본 maximum subarray”와 “현재 index에서 끝나는 maximum subarray”를 유지하는 nonrecursive linear-time algorithm을 유도한다. 즉 maximum-subarray problem은 divide-and-conquer의 힘을 보여 주는 동시에, 문제 구조를 더 깊게 이용하면 $\Theta(n)$까지 갈 수 있음을 보여준다.

### 4.2 Strassen's algorithm for matrix multiplication

#### Straightforward matrix multiplication

두 $n \times n$ matrices $A = (a_{ij})$, $B = (b_{ij})$의 product $C = A \cdot B$에서 entry $c_{ij}$는 다음과 같다.

$$
c_{ij} = \sum_{k=1}^{n} a_{ik} \cdot b_{kj}
$$

각 $c_{ij}$는 $n$개의 product sum이고, entries가 $n^{2}$개 있으므로 straightforward algorithm은 triply nested loops를 가진다.
...
세 loops가 정확히 $n$번씩 돌고 line 7이 constant time이므로 running time은 $\Theta(n^{3})$이다. 직관적으로는 matrix multiplication definition 자체가 $n^{3}$개의 scalar multiplications를 요구하는 것처럼 보이지만, Strassen's algorithm은 이를 깨고 $o(n^{3})$ time이 가능함을 보여준다.

#### Simple divide-and-conquer matrix multiplication

먼저 $n$이 power of 2라고 가정하고, 각 $n \times n$ matrix를 네 개의 $n/2 \times n/2$ submatrices로 나눈다.

$$
\begin{aligned}
A &= [ A11 A12 ] B = [ B11 B12 ] C = [ C11 C12 ] \\
    \text{[ A21 A22 ]        [ B21 B22 ]        [ C21 C22 ]}
\end{aligned}
$$

그러면 $C = A \cdot B$는 네 식으로 나뉜다.

$$
\begin{aligned}
C11 &= A11\cdot B11 + A12\cdot B21 \\
C12 &= A11\cdot B12 + A12\cdot B22 \\
C21 &= A21\cdot B11 + A22\cdot B21 \\
C22 &= A21\cdot B12 + A22\cdot B22
\end{aligned}
$$

각 $C_{ij}$는 two multiplications of $n/2 \times n/2$ matrices와 one matrix addition을 요구한다. 따라서 전체적으로 half-size matrix multiplication이 8번 필요하다.
...
구현상 중요한 subtleness는 partition이다. 실제로 12개의 `n/2 x n/2` matrices를 복사하면 $\Theta(n^{2})$ 시간이 들지만, submatrix를 original matrix의 row/column index range로 표현하면 line 5는 $\Theta(1)$로 처리할 수 있다. 다만 partition copying을 해도 recurrence의 additive term이 $\Theta(n^{2})$로 남으므로 overall asymptotic order는 바뀌지 않는다.
...
Master method로 풀면 $T(n) = \Theta(n^{3})$이다. 즉 naive divide-and-conquer formulation은 straightforward triple loop보다 asymptotically 빠르지 않다.
...
Strassen's method의 핵심은 recursion tree를 “덜 bushy하게” 만드는 것이다. Simple divide-and-conquer는 half-size multiplication을 8번 하지만, Strassen은 이를 7번으로 줄인다. 대신 $n/2 \times n/2$ matrix additions/subtractions를 constant number만큼 더 수행한다. Additions는 전체 recurrence에서 $\Theta(n^{2})$ additive work로 남고, recursive multiplication 수가 8에서 7로 줄어드는 효과가 asymptotic running time을 낮춘다.

Strassen's algorithm은 네 단계로 볼 수 있다.

| 단계 | 내용 | 비용 |
| --- | --- | --- |
| 1 | $A$, $B$, $C$를 $n/2 \times n/2$ submatrices로 나눈다 | $\Theta(1)$ by index calculation |
| 2 | `S1, ..., S10`을 submatrices의 sums/differences로 만든다 | $\Theta(n^{2})$ |
| 3 | `P1, ..., P7`이라는 7개의 recursive matrix products를 계산한다 | $7T(n/2)$ |
| 4 | `P` matrices를 더하고 빼서 `C11, C12, C21, C22`를 만든다 | $\Theta(n^{2})$ |
...
Master method로 이 recurrence의 solution은 $T(n) = \Theta(n^{\lg 7})$이다. $\lg 7$은 2.80과 2.81 사이이므로 $O(n^{2.81})$이라고도 표현한다. 이는 $\Theta(n^{3})$보다 asymptotically faster하다.
...
| --- | --- | --- |
| `C11` | `A11·B11 + A12·B21` | $C11 = A11\cdot B11 + A12\cdot B21$ |
| `C12` | `A11·B12 + A12·B22` | $C12 = A11\cdot B12 + A12\cdot B22$ |
| `C21` | `A21·B11 + A22·B21` | $C21 = A21\cdot B11 + A22\cdot B21$ |
| `C22` | `A21·B12 + A22·B22` | $C22 = A21\cdot B12 + A22\cdot B22$ |
...
| 단계 | 의미 |
| --- | --- |
| 1. Guess the form | recurrence의 solution이 $O(\ldots)$, $\Omega(\ldots)$, $\Theta(\ldots)$ 중 어떤 꼴인지 추측한다 |
| 2. Prove by induction | smaller values에 inductive hypothesis를 적용해 constants를 찾고 bound가 성립함을 보인다 |

“Substitution”이라는 이름은 inductive hypothesis로 얻은 smaller input bound를 recurrence의 $T(\ldots)$ 자리에 대입하기 때문이다. 강력하지만, answer form을 추측할 수 있어야 쓸 수 있다.

#### 예: $T(n) = 2T(\lfloor n/2 \rfloor) + n$
...
Guess는 $T(n) = O(n \lg n)$이다. 이를 증명하려면 적절한 constant $c > 0$에 대해 $T(n) \le c n \lg n$을 보여야 한다. Inductive hypothesis로 모든 positive $m < n$에 대해 bound가 성립한다고 가정하면, 특히 $m = \lfloor n/2 \rfloor$에 대해
...
Substitution proof에서 base case는 조심해야 한다. 위 recurrence에서 $T(1) = 1$이라고 하면 $T(1) \le c \cdot 1 \cdot \lg 1 = 0$이 되어 실패한다. 이는 guess가 틀렸다는 뜻이 아니라, asymptotic bound가 모든 $n \ge 1$에서 성립할 필요는 없다는 뜻이다.

$O$ notation은 $n \ge n_0$ 이후만 요구한다. 원문은 recurrence base case $T(1)$과 inductive proof의 base cases를 구분한다. 예를 들어 $n_0 = 2$로 두고 $T(2)$, $T(3)$을 induction base cases로 삼으면, $n > 3$에서는 recurrence가 직접 $T(1)$에 의존하지 않는다. $T(2) = 4$, $T(3) = 5$를 만족하도록 $c \ge 2$ 정도로 충분히 크게 잡으면 proof가 완성된다.
...
| Heuristic | 설명 |
| --- | --- |
| 비슷한 recurrence와 비교 | $T(n) = 2T(\lfloor n/2 \rfloor + 17) + n$은 $2T(n/2) + n$과 거의 같으므로 $O(n \lg n)$을 guess할 수 있다 |
| loose bounds에서 좁히기 | 먼저 $\Omega(n)$과 $O(n^{2})$ 같은 넓은 bound를 잡고 점차 tight하게 만든다 |
...
에 대해 $T(n) = O(n)$을 guess하고 $T(n) \le cn$을 증명하려 하면,
...
이 되어 $T(n) \le cn$을 얻지 못한다. 하지만 bound 자체는 맞다. 해결법은 lower-order term을 subtract한 stronger hypothesis를 세우는 것이다.
...
Substitution method에서 흔한 오류는 asymptotic notation으로 induction의 exact target을 흐리는 것이다. 예를 들어 위 $T(n) = 2T(\lfloor n/2 \rfloor) + n$에 대해 $T(n) = O(n)$이라고 잘못 증명하려 하면서
...
이라고 쓰면 안 된다. Induction으로 증명해야 할 것은 $T(n) \le cn$이라는 exact form이지, 중간에 다시 $O(n)$으로 뭉개는 것이 아니다. $cn + n$은 $O(n)$이지만 $cn$ 이하는 아니다.
...
에서 $m = \lg n$이라고 두면 $n = 2^{m}$, $\sqrt{n} = 2^{m/2}$이므로
...
원문은 $T(n) = 3T(\lfloor n/4 \rfloor) + \Theta(n^{2})$의 upper bound를 찾기 위해 floors를 생략하고 다음 recurrence를 분석한다.
...
Depth $i$의 subproblem size는 $n/4^{i}$다. $n/4^{i} = 1$이 될 때 $i = \log_{4} n$이므로 tree height는 $\log_4 n$이고, level 수는 $\log_4 n + 1$이다.

Depth $i$의 node 수는 $3^{i}$이고, 각 node cost는 $c(n/4^{i})^{2}$다. 따라서 depth $i$의 total cost는

$$
3^{i} \cdot c(n/4^{i})^{2} = (3/16)^{i} c n^{2}
$$

이다. Bottom level에는 $3^{\log_{4} n} = n^{\log_{4} 3}$ leaves가 있고, 각 leaf cost는 constant이므로 leaf total cost는 $\Theta(n^{\log_{4} 3})$이다.

Internal levels의 cost는 decreasing geometric series다.

$$
\begin{aligned}
\sum_{i=0}^{\log_{4} n - 1} (3/16)^{i} c n^{2} < \sum_{i=0}^{\infty} (3/16)^{i} c n^{2} \\
                                    &= (16/13) c n^{2}
\end{aligned}
$$

따라서 total cost는 $O(n^{2})$이다. Root cost $cn^{2}$가 total cost의 constant fraction을 차지하므로 root dominates the tree라고 볼 수 있다. 또한 첫 recursive expansion 자체에 $\Theta(n^{2})$ cost가 있으므로 lower bound도 $\Omega(n^{2})$이고, 결국 tight bound는 $\Theta(n^{2})$다.

Substitution으로 upper bound를 확인하면 다음과 같다.

$$
\begin{aligned}
T(n) &\le 3T(\lfloor n/4 \rfloor) + c n^{2} \\
     &\le 3d\lfloor n/4 \rfloor^{2} + c n^{2} \\
     &\le (3/16)d n^{2} + c n^{2} \\
     &\le d n^{2} if d \ge (16/13)c
\end{aligned}
$$
...
$$
(2/3)^{k} n = 1 \Rightarrow k = \log_{3/2} n
$$
...
Substitution으로 $T(n) \le d n \lg n$을 검증하면:
...
마지막 부등식은 $d \ge c / (\lg 3 - 2/3)$이면 성립한다. 따라서 $O(n \lg n)$ upper bound가 검증된다.
...
여기서 $a \ge 1$, $b > 1$은 constants이고, $f(n)$은 asymptotically positive function이다. 이 recurrence는 size $n$ 문제를 $a$개의 subproblems로 나누고, 각 subproblem size가 $n/b$이며, divide와 combine의 전체 cost가 $f(n)$인 divide-and-conquer algorithm을 나타낸다.

예를 들어 Strassen's algorithm은 $a = 7$, $b = 2$, $f(n) = \Theta(n^{2})$이다.

Floors와 ceilings는 보통 생략한다. 실제로는 $T(\lfloor n/b \rfloor)$ 또는 $T(\lceil n/b \rceil)$가 되어야 하지만, master theorem은 이런 rounding이 asymptotic behavior를 바꾸지 않음을 보장한다.

#### Theorem 4.1: Master theorem

Master theorem은 $f(n)$과 $n^{\log_{b} a}$를 비교한다. 여기서 $n^{\log_{b} a}$는 recursion leaves 또는 recursive subproblem proliferation이 만들어 내는 기준 성장률이다.

| Case | 조건 | 결과 | 직관 |
| --- | --- | --- | --- |
| 1 | $f(n) = O(n^{\log_{b} a - \varepsilon})$ for some $\varepsilon > 0$ | $T(n) = \Theta(n^{\log_{b} a})$ | recursive subproblems 쪽이 지배 |
| 2 | $f(n) = \Theta(n^{\log_{b} a})$ | $T(n) = \Theta(n^{\log_{b} a} \lg n)$ | 각 recursion level이 비슷하게 기여 |
| 3 | $f(n) = \Omega(n^{\log_{b} a + \varepsilon})$ for some $\varepsilon > 0$, and $a f(n/b) \le c f(n)$ for some $c < 1$ | $T(n) = \Theta(f(n))$ | root/combine work 쪽이 지배 |

Case 3의 추가 조건 $a f(n/b) \le c f(n)$은 regularity condition이다. 이는 $f(n)$이 충분히 규칙적으로 커져서 상위 level의 work가 하위 level들의 total work보다 일정 비율 이상 크다는 뜻이다. 원문은 우리가 만나는 대부분의 polynomially bounded functions에서는 이 조건이 만족된다고 설명한다.

#### Polynomially smaller/larger 조건

Master theorem의 세 cases는 단순히 $f(n)$이 작다/같다/크다만 보지 않는다.

- Case 1에서는 $f(n)$이 $n^{\log_{b} a}$보다 polynomially smaller해야 한다. 즉 어떤 $\varepsilon > 0$만큼 지수 차이가 있어야 한다.
- Case 3에서는 $f(n)$이 $n^{\log_{b} a}$보다 polynomially larger해야 하며, regularity condition도 만족해야 한다.
...
여기서 $a = 2$, $b = 2$, $n^{\log_{b} a} = n$, $f(n) = n \lg n$이다. $f(n)$은 $n$보다 asymptotically larger이지만, ratio가 $\lg n$뿐이라 어떤 $n^\varepsilon$보다도 작다. 따라서 Case 3의 polynomially larger 조건을 만족하지 못하고, master method를 바로 적용할 수 없다.

#### 사용 예시

Master method는 $a$, $b$, $f(n)$을 확인하고 $f(n)$을 $n^{\log_{b} a}$와 비교하면 된다.

| Recurrence | $a$, $b$, $f(n)$ | 비교 | Case | Solution |
| --- | --- | --- | --- | --- |
| $T(n) = 9T(n/3) + n$ | $a=9$, $b=3$, $f(n)=n$ | $n^{\log_{3} 9}=n^{2}$, $f(n)$ smaller | 1 | $\Theta(n^{2})$ |
| $T(n) = T(2n/3) + 1$ | $a=1$, $b=3/2$, $f(n)=1$ | $n^{\log_{3/2}1}=1$, same | 2 | $\Theta(\lg n)$ |
| $T(n) = 3T(n/4) + n \lg n$ | $a=3$, $b=4$, $f(n)=n \lg n$ | $n^{\log_{4} 3}=O(n^{0}.793)$, $f(n)$ larger and regular | 3 | $\Theta(n \lg n)$ |
...
여기서 $a = 2$, $b = 2$, $n^{\log_{b} a} = n$, $f(n) = \Theta(n)$이므로 Case 2다.
...
이고 $a = 8$, $b = 2$, $n^{\log_{2} 8} = n^{3}$이다. $f(n) = \Theta(n^{2})$는 polynomially smaller이므로 Case 1이다.
...
이고 $a = 7$, $b = 2$, $n^{\log_{2} 7} = n^{\lg 7}$이다. $2.80 < \lg 7 < 2.81$이고 $f(n)=\Theta(n^{2})$는 $n^{\lg 7}$보다 polynomially smaller이므로 Case 1이다.

$$
T(n) = \Theta(n^{\lg 7}) = O(n^{2}.81)
$$

이 결과가 Strassen이 straightforward $\Theta(n^{3})$ matrix multiplication보다 asymptotically faster한 이유다.

### 4.6 Proof of the master theorem

이 절은 `master theorem`을 적용하는 데 필수는 아니지만, 왜 세 case가 생기는지 설명한다. 증명은 두 단계다. 먼저 $n$이 정확히 $b$의 거듭제곱(exact powers of $b$)일 때 recurrence tree를 분석한다. 그다음 실제 알고리즘처럼 $\lfloor n/b \rfloor$, $\lceil n/b \rceil$가 들어가는 모든 양의 정수 $n$으로 확장한다.

주의할 점은 제한된 domain에서의 asymptotic notation이다. 예를 들어 어떤 함수가 $n=1,2,4,8,\ldots$에서만 $O(n)$이어도, 다른 $n$에서 $n^{2}$처럼 정의되어 있다면 전체 domain에서는 $O(n)$이라고 말할 수 없다. 그래서 proof는 exact powers에서 직관을 얻되, 마지막에 floor/ceiling을 따로 처리한다.
...
여기서 $a \ge 1$, $b > 1$, $f(n)$은 nonnegative function이고, 우선 $n = b^{i}$라고 가정한다. Lemma 4.2는 이 recurrence를 직접 푸는 대신, recursion tree의 level cost 합으로 바꾼다.
...
이다. leaf는 $n/b^{j} = 1$이 되는 depth, 즉 $j = \log_{b} n$에 있고, leaf 수는

$$
a^{\log_{b} n} = n^{\log_{b} a}
$$

개다. leaf 하나의 cost는 $\Theta(1)$이므로 leaf 전체 cost는 $\Theta(n^{\log_{b} a})$다. 따라서 exact powers에서 전체 시간은
...
| case | tree 관점 | 결과 |
| --- | --- | --- |
| Case 1 | $f(n)$이 leaf 기준 $n^{\log_{b} a}$보다 polynomially smaller라서 leaves가 지배 | $\Theta(n^{\log_{b} a})$ |
| Case 2 | 각 level의 총 cost가 거의 같아서 level 수만큼 누적 | $\Theta(n^{\log_{b} a} \lg n)$ |
| Case 3 | root 쪽 cost $f(n)$이 충분히 커서 위쪽 level이 지배 | $\Theta(f(n))$ |

Lemma 4.3은 summation

$$
g(n) = \sum_{j=0}^{\log_{b} n - 1} a^{j} f(n/b^{j})
$$

을 세 상황에서 평가한다.

Case 1에서는 $f(n)=O(n^{\log_{b} a - \varepsilon})$이다. $f(n/b^{j})$를 대입하면 level cost가 geometric series 형태가 되고, 전체 internal cost는 $O(n^{\log_{b} a})$를 넘지 않는다. leaf cost도 $\Theta(n^{\log_{b} a})$이므로 최종적으로 $T(n)=\Theta(n^{\log_{b} a})$다.

Case 2에서는 $f(n)=\Theta(n^{\log_{b} a})$이다. 이때 각 level의 cost가 모두 같은 차수다.

$$
\begin{aligned}
a^{j} (n/b^{j})^{\log_{b} a} \\
= n^{\log_{b} a} (a / b^{\log_{b} a})^{j} \\
= n^{\log_{b} a}
\end{aligned}
$$

level이 $\log_b n$개 있으므로 internal cost는 $\Theta(n^{\log_{b} a} \lg n)$이고, leaf cost보다 크거나 같은 차수라서 전체도 $\Theta(n^{\log_{b} a} \lg n)$이다.

Case 3에서는 regularity condition

$$
a f(n/b) \le c f(n) for some constant c < 1
$$
...
$$
g(n) \le f(n) \sum c^{j} + O(1) = O(f(n))
$$

또한 summation의 첫 항이 $f(n)$이므로 $g(n)=\Omega(f(n))$이고, 결국 $g(n)=\Theta(f(n))$이다. 여기에 $f(n)=\Omega(n^{\log_{b} a+\varepsilon})$가 붙으면 leaf cost $\Theta(n^{\log_{b} a})$는 $f(n)$보다 작아서 $T(n)=\Theta(f(n))$가 된다.

#### 4.6.2 Floors and ceilings

실제 recurrence는 보통 subproblem size가 정확히 $n/b$가 아니라 $\lfloor n/b \rfloor$ 또는 $\lceil n/b \rceil$다.
...
\begin{aligned}
n_{0} &= n \\
n_{j} &= \lceil n_{j-1}/b \rceil for j > 0
\end{aligned}
$$

로 두면, $\lceil x \rceil \le x + 1$이므로 다음 bound를 얻는다.

$$
n_{j} < n/b^{j} + b/(b-1)
$$

$j = \lfloor \log_{b} n \rfloor$이면 $n_{j} = O(1)$이므로 tree depth는 여전히 $\Theta(\lg n)$이다. 따라서 ceiling이 있어도 tree는 대략 Figure 4.8처럼 읽을 수 있다.

![Figure 4.8](@/assets/images/cs-algorithm-015-figure-4-8-page-125.png)
*Figure 4.8 · PDF p. 125 · $T(n)=aT(\lceil n/b \rceil)+f(n)$에서 recursive argument $n_{j}$를 쓰는 recursion tree*

전체 cost는 exact powers의 식과 거의 같은 형태가 된다.

$$
T(n) = \Theta(n^{\log_{b} a}) + \sum_{j=0}^{\lfloor \log_{b} n \rfloor - 1} a^{j} f(n_{j})
$$

Case 3에서는 regularity condition을 $a f(\lceil n/b \rceil) \le c f(n)$ 형태로 쓰면 $a^{j} f(n_{j}) \le c^{j} f(n)$이 되어 exact powers proof와 같은 decreasing geometric series가 나온다. Case 2에서는 $n_{j} < n/b^{j} + b/(b-1)$를 이용해 $f(n_{j})=O(n^{\log_{b} a}/a^{j})$를 보이면 각 level cost가 $O(n^{\log_{b} a})$로 유지된다. Case 1도 같은 구조지만 지수에 $-\varepsilon$가 들어가 algebra가 조금 더 복잡할 뿐이다.
...
$n^{\log_{b} a}$는 leaf가 얼마나 많아지는지를 나타낸다. 이것은 “recursive branching이 만들어내는 일의 양”이다. 반대로 $f(n)$은 각 node에서 recursive calls 바깥으로 수행하는 divide/combine cost다. master method는 이 둘의 성장률을 비교해 tree의 어느 부분이 총합을 지배하는지 판단한다.

| 판단 질문 | 의미 |
| --- | --- |
| $f(n)$이 $n^{\log_{b} a}$보다 polynomially smaller인가? | leaves가 지배하므로 Case 1 |
| $f(n)$이 $n^{\log_{b} a}$와 같은 차수인가? | 모든 level이 비슷하게 기여하므로 Case 2 |
...
예를 들어 $T(n)=4T(n/2)+n^{2} \lg n$에서는 $n^{\log_{2} 4}=n^{2}$이고 $f(n)=n^{2} \lg n$이다. $f(n)$은 $n^{2}$보다 크지만 polynomially larger가 아니라 logarithmic factor만 크다. 따라서 기본 master theorem의 Case 3으로 곧장 처리할 수 없다. 이런 경우에는 recursion tree, substitution method, 또는 확장된 master theorem이 필요하다.
...
- $4.5-2$는 Strassen보다 빠른 matrix multiplication을 만들려면 $T(n)=aT(n/4)+\Theta(n^{2})$에서 $a$가 얼마나 작아야 하는지 묻는다. 비교 대상은 $\Theta(n^{\lg 7})$이므로 $n^{\log_{4} a}$와 $n^{\lg 7}$를 비교해야 한다.
- $4.5-4$, $4.6-2$는 $f(n)=n^{\log_{b} a} \lg^{k} n$처럼 boundary case에 logarithmic factor가 붙을 때 기본 master theorem만으로는 부족하다는 점을 보여준다.
- $4.6-3$은 Case 3의 regularity condition이 단순한 부가 조건이 아니라 $f(n)$이 실제로 $n^{\log_{b} a}$보다 polynomially 커지는 성질과 연결된다는 점을 확인하게 한다.
- `4-2 Parameter-passing costs`는 알고리즘의 abstract recurrence가 programming model assumption에 의존함을 보여준다. 배열을 pointer로 넘기면 procedure call cost는 $\Theta(1)$이지만, array copying을 하면 recurrence의 $f(n)$이 달라진다.
...
마지막으로 master method는 간단하지만 equal subproblem size recurrence에 맞춰져 있다. $T(n)=T(\lfloor n/3 \rfloor)+T(\lfloor 2n/3 \rfloor)+O(n)$ 같은 recurrence는 master method로 직접 풀 수 없고, 더 일반적인 `Akra-Bazzi method`가 필요하다. Akra-Bazzi는 여러 비율의 subproblems를 허용하며, $\sum a_{i} b_{i}^{p} = 1$을 만족하는 `p`를 찾은 뒤 integral term으로 해를 표현한다.

## 복잡도

| 대상 | recurrence / bound | 핵심 이유 |
| --- | --- | --- |
| Maximum-subarray divide-and-conquer | $T(n)=2T(n/2)+\Theta(n)$ → $\Theta(n \lg n)$ | left/right/crossing 세 후보, crossing 계산이 linear |
| Simple matrix multiplication | $T(n)=8T(n/2)+\Theta(n^{2})$ → $\Theta(n^{3})$ | 8 recursive multiplications plus matrix add |
| Strassen's algorithm | $T(n)=7T(n/2)+\Theta(n^{2})$ → $\Theta(n^{\lg 7})$ | 1 recursive multiplication을 14회 add/subtract와 교환 |
...
| Master method Case 1 | $f(n)=O(n^{\log_{b} a-\varepsilon})$ → $\Theta(n^{\log_{b} a})$ | leaves dominate |
| Master method Case 2 | $f(n)=\Theta(n^{\log_{b} a})$ → $\Theta(n^{\log_{b} a} \lg n)$ | all levels balanced |
| Master method Case 3 | $f(n)=\Omega(n^{\log_{b} a+\varepsilon})$ + regularity → $\Theta(f(n))$ | root/top levels dominate |

## 연결 관계

- Chapter 2의 `insertion sort`, `merge sort`, `loop invariant`와 연결된다. 특히 `merge sort` recurrence $T(n)=2T(n/2)+\Theta(n)$은 Chapter 4에서 recursion tree와 master method로 다시 분석된다.
- Chapter 3의 `asymptotic notation`, `polynomially larger/smaller`, $\lg n$, $\Theta$, $O$, $\Omega$ 없이는 master theorem의 조건을 정확히 읽을 수 없다.
...
- Divide-and-conquer가 항상 빠른 것은 아니다. maximum-subarray는 divide-and-conquer로 $\Theta(n \lg n)$이지만 linear-time algorithm도 존재한다.
...
- $f(n)=n^{\log_{b} a} \lg n$은 $n^{\log_{b} a}$보다 크지만 polynomially larger는 아니다. 기본 master theorem의 세 case 중 하나에 자동으로 들어가지 않는다.
...
3. Strassen's algorithm이 $\Theta(n^{3})$보다 빠른 이유를 recurrence 관점에서 설명하라.
4. $T(n)=3T(n/4)+cn^{2}$를 recursion tree로 분석하면 왜 $\Theta(n^{2})$가 되는가?
5. Substitution method에서 lower-order term 때문에 induction이 실패할 때 어떻게 stronger hypothesis를 세우는가?
6. Master theorem의 $n^{\log_{b} a}$는 recursion tree에서 무엇을 의미하는가?
